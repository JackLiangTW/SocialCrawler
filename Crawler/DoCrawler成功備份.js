...
const loopdata=[
  {name:'CP0',url:'https://www.facebook.com/pg/KATOTAKA2.0/posts/'},
  {name:'CP1',url:'https://www.facebook.com/pg/%E7%88%86%E6%96%99%E5%85%AC%E7%A4%BE-162608724089621/posts'},
  {name:'CP2',url:'https://www.facebook.com/pg/appledaily.tw/posts/?ref=page_internal'},
  //{name:'CP3',url:'https://www.facebook.com/pg/%E5%A6%B9%E5%AD%90%E5%B0%88%E5%8D%80-823320937840113/posts/'},
  {name:'CP4',url:'https://www.facebook.com/pg/bclowy/posts/?ref=page_internal'},
  {name:'CP5',url:'https://www.facebook.com/groups/256720108361190/?ref=group_browse'},  
];

//ManyCraynamic_puppeteer();
Craynamic_puppeteer();

function ManyCraynamic_puppeteer(){//--puppeteer套件 動態抓取/最好的
  (async () => {
    ...
    while(control<loopdata.length){
        ...
        const values = await page.evaluate(//get data-utime OK
            () =>
...
function Craynamic_puppeteer(){//--puppeteer套件 動態抓取/最好的
  (async () => {
    ...
    const page = await browser.newPage();
    await page.goto(loopdata[4].url);
    await page.setViewport({width:1000, height: 800});//--設定開啟虛擬瀏覽器寬高
...
/*OR 取得貼文 的attributes
        let bodyHTML = await page.$$eval('._14i5 ._5eit abbr', el => {//get only title OK
        ...
        const values = await page.evaluate(//get data-utime OK
            () =>
                [...document.querySelectorAll('._14i5 ._5eit abbr')]                  
                .map(function(i) {                        
                        var OBJ = {};
                        OBJ['date'] = i.title;
                        OBJ['nb'] = i.getAttribute('data-utime');
                        OBJ['child'] = i.childNodes[0].getAttribute('class');//抓><span>                        
                        return OBJ;
                })
        );
        */